...
- [ ] **Step 3: Implement** `revealGate.ts`:
  ...
  - `isToolExecution(ctx)`: try — if `ctx.getNode()?.parameters?.__isToolCall` or `ctx.additionalData?.isToolExecution` or `ctx.additionalData?.aiTool` → `true`; `mode = ctx.getMode?.()`; if `mode && SAFE_MODES.includes(mode)` → `false`; else `true`. Any throw → `true`. (Fail closed; document: confirm positive-signal field names against the pinned `n8n-workflow` version during integration, but the contract — *inconclusive ⇒ blocked* — is fixed and tested here.)
